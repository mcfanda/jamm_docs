...
For models without interactions, jAMM produces a table that show the `Indirect effects` (mediated), the `Direct effects`, and the `Total effects`. The example below shows results of a model with one IV (`x1`), two parallel mediators (`m1`, `m2`) and a DV (`y`).
...
This features allows users to define an interaction effect that will be treated as an additional term of the mediation model. Thus, no simple indirect effects will be computed. Nontheless, the model may be perfectly fine and the user may have good reasons to insert an interaction without explicitely define a moderator. The feature also allows estimating very complex models with several interactions, not all of them interesting from the point of view of the mediation effects. 
